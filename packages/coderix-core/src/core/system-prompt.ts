...
export class SystemPromptAssembler {
  ...
  private buildSystemRules(role: string): PromptPart | null {
    ...
    const rules = [
      ...
      '# Do not stop early',
      '',
      'Never end a turn while the task is unfinished. A plan, an intention, or a',
      'description of your next step is not the completion of the task — it is the cue to',
      'do it now.',
      '',
      'If you are about to write "I will do X", "next I will Y", "I will switch to Z", or',
      'any statement of what comes next, do not stop there: perform the tool call that does',
      'X / Y / Z in this same turn.',
      '',
      'When a tool fails, do not stop at announcing an alternative — actually try the',
      'alternative in the same turn.',
      '',
      'You may end a turn only when:',
      '- the task is genuinely complete and you have verified it works, or',
      '- you truly need the user to answer a question and no further work is possible',
      '  without that answer.',
      '',
      'Anything else — a plan, a partial result, or a failed attempt with an un-executed',
      'fallback — means you are not done. Keep going.',
      '',
      '# Debugging',
      '',
