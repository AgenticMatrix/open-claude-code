...
declare global {
  ...
  interface Window {
    coderixAPI: {
      query: { submit(query: string, sessionId?: string, skills?: string[]): Promise<{ status: string }>; interrupt(sessionId?: string): Promise<{ status: string }> };
      session: { create(opts?: { title?: string; cwd?: string; model?: string }): Promise<{ id: string; title: string; turnCount: number; cwd: string }>; list(): Promise<unknown[]>; get(sessionId: string): Promise<unknown>; load(sessionId: string): Promise<unknown>; fork(sessionId: string): Promise<unknown>; delete(sessionId: string): Promise<{ status: string }>; setModel(sessionId: string, model: string): Promise<{ status: string; model: string }>; setSkills(sessionId: string, skills: string[]): Promise<{ status: string; skills: string[] }> };
      ...
      defaultWorkspace: { get(): Promise<{ path: string }>; set(path: string): Promise<{ path: string }>; select(): Promise<{ canceled: boolean; path: string }> };
